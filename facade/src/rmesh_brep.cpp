...
struct IrBuilder {
    ...
    // Assign IR indices in the kernel's OWN sub-shape order.
    //
    // Without this, indices follow the order entities are first met while
    // walking solids -> shells -> faces -> wires -> edges, while every kernel
    // API that takes a sub-shape (`occt_get_sub_shapes`, and therefore every
    // edge a caller can name) enumerates with `TopExp::MapShapes`. Those are
    // different permutations, so "edge 3 of the lifted model" and "edge 3 of
    // the kernel's shape" were different edges -- a host resolving a reference
    // against the IR and handing the index back to the kernel silently
    // operated on the WRONG entity. Measured on a 3x4x5 box: 7 of 12 edges
    // disagreed even by length class.
    //
    // Seeding the maps in MapShapes order makes the two enumerations the same
    // enumeration by construction, rather than two orders that have to be
    // kept in agreement.
    void seed_order(const TopoDS_Shape& shape) {
        ShapeMap ordered_vertices;
        TopExp::MapShapes(shape, TopAbs_VERTEX, ordered_vertices);
        for (int i = 1; i <= ordered_vertices.Extent(); ++i) {
            admit_vertex(TopoDS::Vertex(ordered_vertices.FindKey(i)));
        }
        ShapeMap ordered_edges;
        TopExp::MapShapes(shape, TopAbs_EDGE, ordered_edges);
        for (int i = 1; i <= ordered_edges.Extent(); ++i) {
            const TopoDS_Edge edge = TopoDS::Edge(ordered_edges.FindKey(i));
            // Skip degenerate edges exactly as `emit_loop` does: a pole seam
            // has no 3D curve to lift. The IR's edge enumeration is therefore
            // "MapShapes order, minus degenerate", which is precisely what
            // `occt_non_degenerate_edges` hands a caller -- so the two agree
            // for shapes with poles (every filleted corner has three).
            if (BRep_Tool::Degenerated(edge)) {
                continue;
            }
            admit_edge(edge);
        }
        ShapeMap ordered_faces;
        TopExp::MapShapes(shape, TopAbs_FACE, ordered_faces);
        for (int i = 1; i <= ordered_faces.Extent(); ++i) {
            admit_face(TopoDS::Face(ordered_faces.FindKey(i)));
        }
    }
...
int32_t occt_to_brep_ir(uint32_t shape_id) {
    ...
    try {
        ...
        IrBuilder builder;
        // Adopt the kernel's sub-shape order BEFORE the structural walk, so
        // IR indices and kernel sub-shape indices are the same numbers.
        builder.seed_order(shape);
