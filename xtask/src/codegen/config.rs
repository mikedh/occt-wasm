...
return store(normalizeSolidOrientation(maker.Shape()));",
    ...
    MethodSpec {
        ...
        // NOT TopoDS::Solid(...): BRepFilletAPI_MakeFillet takes a
        // TopoDS_Shape, and narrowing it here threw Standard_TypeMismatch for
        // every shape that was not literally a TopoDS_SOLID -- which includes
        // the TopoDS_COMPOUND every BRepAlgoAPI boolean returns. Filleting
        // anything that had been cut or fused was therefore impossible.
        ctor_args: "get(solidId)",
        setup_code: "",
        includes: &[],
        category: "modeling",
        return_type: ReturnType::ShapeId,
    ...
    MethodSpec {
        ...
        // See `fillet`: the builder takes a TopoDS_Shape.
        ctor_args: "get(solidId)",
        setup_code: "",
        includes: &[],
        category: "modeling",
...
return store(maker.Shape());",
    ...
    MethodSpec {
        ...
        ctor_args: "",
        setup_code: "\
BRepFilletAPI_MakeFillet maker(get(solidId));
...
for (size_t i = 0; i < solidIds.size(); i++) {
    BRepFilletAPI_MakeFillet maker(get(solidIds[i]));
...
return result;",
    ...
    MethodSpec {
        // The edge enumeration the IR lift uses: `TopExp::MapShapes` order
        // with degenerate edges removed.
        //
        // `getSubShapes(id, "edge")` returns every edge INCLUDING pole seams,
        // which carry no 3D curve and so cannot appear in a boundary-
        // representation IR. A caller that resolves a reference against the IR
        // and hands the index back would then be off by the number of
        // degenerate edges preceding it -- and every filleted corner has
        // three. This is the enumeration both sides can agree on.
        name: "nonDegenerateEdges",
        kind: MethodKind::CustomBody,
        params: &[FacadeParam::ShapeId("id")],
        occt_class: "",
        ctor_args: "",
        setup_code: "\
NCollection_IndexedMap<TopoDS_Shape, TopTools_ShapeMapHasher> map;
TopExp::MapShapes(get(id), TopAbs_EDGE, map);
std::vector<uint32_t> result;
for (int i = 1; i <= map.Extent(); i++) {
    ...
    result.push_back(store(edge));
}
return result;",
        includes: &[
            "TopExp.hxx", "TopAbs_ShapeEnum.hxx", "NCollection_IndexedMap.hxx",
            "TopTools_ShapeMapHasher.hxx", "BRep_Tool.hxx", "TopoDS.hxx",
        ],
        category: "topology",
        return_type: ReturnType::VectorUint32,
    },
...
return checker.IsValid();",
    ...
    MethodSpec {
        // BRepCheck_Analyzer (isValid) does NOT detect self-intersection --
        // a blend or boolean can pass it and still hand back a shape whose
        // faces cross each other. BRepAlgoAPI_Check adds exactly that, plus
        // small-edge detection, and is the check a caller wants before
        // treating a result as a real solid.
        name: "checkShape",
        kind: MethodKind::CustomBody,
        params: &[FacadeParam::ShapeId("id")],
        occt_class: "",
        ctor_args: "",
        setup_code: "\
BRepAlgoAPI_Check checker(get(id), Standard_True /*testSelfInt*/, Standard_False /*testSmallEdge*/);
checker.Perform();
return checker.IsValid();",
        includes: &["BRepAlgoAPI_Check.hxx"],
        category: "healing",
        return_type: ReturnType::Bool,
    },
    MethodSpec {
