...
pub fn emit_wasi_exports(methods: &[&MethodSpec]) -> String {
    ...
    // A struct-accessor group is emitted only when some spec in this emission
    // actually returns its type. For the full spec set that is every group;
    // the `--minimal` profile derives its export roots from a core-only
    // emission, where an accessor group with no producer would root dead code.
    let uses = |rt: ReturnType| methods.iter().any(|m| m.return_type == rt);
    ...
    if uses(ReturnType::MeshBatchData) {
    ...
    if uses(ReturnType::NurbsCurveData) {
    ...
    if uses(ReturnType::EvolutionData) {
    ...
    if uses(ReturnType::ProjectionData) {
    ...
    if uses(ReturnType::XCAFLabelInfo) {
...
/// Extract `occt_*` export names from C++ source by scanning for
/// `<type> occt_<name>(` patterns. Works on the generated `wasi_exports.cpp`,
/// hand-written facade extensions, and in-memory emissions alike — the
/// `--minimal` export-root derivation scrapes an in-memory core-only emission
/// with exactly this function, so the root set cannot drift from the emitter.
pub fn export_names(source: &str) -> Vec<String> {
...
mod tests {
    ...
    #[test]
    fn accessor_groups_follow_return_types_in_use() {
        use super::super::types::{MethodKind, MethodSpec, ReturnType};

        // No spec produces ProjectionData/XCAFLabelInfo → their accessor
        // groups must not be emitted (they would root dead code in --minimal).
        let output = emit_wasi_exports(&[]);
        assert!(!output.contains("occt_get_proj_"));
        assert!(!output.contains("occt_get_label_info_"));

        static PROJECT_EDGES: MethodSpec = MethodSpec {
            name: "projectEdges",
            kind: MethodKind::CustomBody,
            params: &[],
        ...
        let output = emit_wasi_exports(&[&PROJECT_EDGES]);
        assert!(output.contains("occt_get_proj_visible_outline"));
        assert!(!output.contains("occt_get_label_info_"));
    }

    #[test]
    fn export_names_scrapes_definitions() {
        let names = export_names(
            "uint32_t occt_make_box(double dx) {\n\
             // occt_ mentioned in a comment (\n\
             int32_t occt_init() { return 0; }\n\
             uint32_t occt_make_box(double dx); // dup declaration\n",
        );
        assert_eq!(
            names,
            vec!["occt_init".to_owned(), "occt_make_box".to_owned()]
        );
    }
}
